...
try {
    ...
    header('Location: index.php?error=Ошибка базы данных');
    exit;
}
        ...
        <div class="form-container">
            <form method="POST" action="edit.php?id=<?= $id ?>">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id = "patronymic" name="patronymic" value="<?=htmlspecialchars($_POST['patronymic']  ?? $employee['patronymic'] ?? '') ?>" required>
                </div>

                <div class="form-group">
                    <label for = "birth_date">Дата рождения </label>
                    <input type="date" id = "birth_date" name="birth_date" value="<?=htmlspecialchars($_POST['birth_date'] ?? $employee['birth_date'] ?? '') ?>" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="passport" name="passport" 
                           ...
                           value="<?=htmlspecialchars($_POST['passport'] ?? $employee['passport'] ?? '') ?>" required>
                </div>

                <div class="form-group">
                    <label for = "phone">Телефон </label>
                    <input type="tel" id = "phone" name="phone" placeholder="+7 (999) 123-45-67" value="<?=htmlspecialchars($_POST['phone'] ?? $employee['phone'] ?? '') ?>" required>
                </div>

                <div class="form-group">
                    <label for = "address">Адрес проживания </label>
                    <textarea id="address" name="address" required><?= htmlspecialchars($_POST['address'] ?? $employee['address'] ?? '') ?></textarea>
                </div>
                    ...
                    <select id="department_id" name="department_id" required>
                        ...
                        <?php foreach ($departments as $dept): ?>
                            <option value="<?= $dept['id'] ?>" 
                                <?= ($_POST['department_id'] ?? $employee['department_id'] ?? '') == $dept['id'] ? 'selected' : '' ?>>
                ...
                <div class="form-group">
                    ...
                    <select id="position_id" name="position_id" required>
                        ...
                        <?php foreach ($positions as $pos): ?>
                            <option value="<?= $pos['id'] ?>" 
                                <?= ($_POST['position_id'] ?? $employee['position_id'] ?? '') == $pos['id'] ? 'selected' : '' ?>>
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="salary" name="salary" step="0.01" min="0" value="<?= htmlspecialchars($_POST['salary'] ?? $employee['salary'] ?? '') ?>" required>
                </div>

                <div class="form-group">
                    <label for="hire_date">Дата приема </label>
                    <input type="date" id="hire_date" name="hire_date" 
                           value="<?= htmlspecialchars($_POST['hire_date'] ?? $employee['hire_date'] ?? '') ?>" required>
                </div>
